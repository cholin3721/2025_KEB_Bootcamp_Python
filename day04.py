...
def print_bill() :
    ...
    print("영수증")
    print("*" * 20)
    for a, b in bill.items() :
        print(f"{a} + {drinks_foods[drinks.index(a)]}\n {b:<2} 세트 | {b * prices[(drinks.index(a))]:<10}")
        amount +=b * prices[(drinks.index(a))]
    print(f"\n지불 금액 : {amount} 원")
    print("*" * 20)
    print("감사합니다")
    print()
...
def make_bill(menu) :
    if drinks[menu] in list(bill.keys()) :
        bill[drinks[menu]] += 1
    else :
        bill[drinks[menu]] = 1
...
def print_menu(drink, food) :
    print_list = [(a, b, f"가격 : {c}원") for a, b, c in zip(drink, food, prices)]
...
def modify(drink, food, pick) :
    ...
    choice = input("가격도 수정하시겠어요? (Y/N)").strip().upper()
    if choice == 'Y' :
        price = int(input("얼마로 수정하시겠어요 ? : "))
        prices[pick] = price
    else :
        print("수정을 종료합니다.")
        return
...
while True:
    ...
    for i, drink in enumerate(drinks) :
        ...
        j = i
    menu_list += f'\n\n{j+2}.아무거나, {j+3}.이건 아니지! 수정 {j+4}.이게 없네~ 추가 {j+5}.넌 나가라~ 제거 {j+6}.전체 출력!'
    menu_list += f"\n\n{j+7}.주문 내역 {j+8}.종료 "
    try :
    ...
    if menu >= 1 and menu <= len(drinks):
        how_bout_this(drinks, drinks_foods, menu-1)
